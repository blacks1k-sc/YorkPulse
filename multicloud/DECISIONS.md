...
## Decision 016 — Azure Container Apps over AKS for the Next.js Frontend

**Decision**: Deploy the Next.js SSR frontend on Azure Container Apps, not Azure Kubernetes Service (AKS).

**Options Considered**:
1. Azure Container Apps — serverless, managed Kubernetes under the hood, no cluster to operate
2. AKS — full Kubernetes cluster, maximum control, industry-standard for complex workloads
3. Azure App Service — PaaS for web apps, simpler than Container Apps but less portable
4. Azure Static Web Apps — JAMstack hosting, does NOT support Next.js SSR (rejected outright — see Rule 6 in CLAUDE.md)

**Why This Choice**:
The frontend is a single stateless Next.js SSR container. It has no sidecars, no inter-service
mesh requirements, no custom ingress controllers, and no persistent volumes. These are the
workload characteristics that AKS was designed for — and none of them apply here.

Container Apps eliminates the entire cluster management layer:
- No node pools to size, patch, or upgrade
- No kubeconfig, kubectl, or Helm charts to maintain
- No control plane cost (~$0.10/hour on AKS, ~$0 on Container Apps at low traffic)
- TLS termination and HTTPS assignment happen automatically
- Scale-to-zero (optional) and HTTP-triggered autoscaling are built in

The trade-off: less control. Container Apps abstracts away the Kubernetes layer, so if a
workload needed custom RBAC, network policies, or a service mesh, Container Apps would be
insufficient. For a single Next.js frontend that serves HTTP traffic and nothing else, that
abstraction is the correct level.

**Tradeoffs**:
AKS is the right choice when:
- You have multiple microservices that need to discover and communicate with each other
- You need custom ingress controllers (Nginx, Traefik) or network policies
- Your team already has Kubernetes expertise and tooling
- You need fine-grained pod-level resource controls

Container Apps is the right choice here:
- One service, one container, HTTP ingress — that's the whole workload
- No Kubernetes expertise needed to operate it
- ~$0 management overhead vs ~$73/month minimum for a 2-node AKS cluster

**Interview Talking Point**:
"Azure Container Apps runs on Kubernetes under the hood — Microsoft manages the control plane.
I chose it over AKS because the workload is a single stateless SSR frontend with no inter-service
dependencies. AKS is appropriate for complex microservices requiring service mesh, custom
ingress, or fine-grained network policies. Running AKS for one container is like buying a
semi-truck to deliver one pizza. Container Apps gives me managed scaling, automatic TLS,
and zero cluster overhead at a fraction of the cost."

---

## Decision 017 — Personal Azure Account Over York University Tenant

**Decision**: Use a personal Azure for Students account (nrup1618@outlook.com) instead of
the York University Azure tenant for all YorkPulse cloud infrastructure.

**Options Considered**:
1. York University tenant — already have access, no new account needed
2. Personal Azure for Students — free $100 credit, full owner permissions, persists after graduation
3. Pay-As-You-Go personal account — full control, billing after $0 free tier

**Why This Choice**:
The York University Azure tenant enforces IT-managed Azure Policies that block creation of most
resource types, including Resource Groups in non-approved regions, Container Apps (Microsoft.App),
Container Registries, and Key Vaults. These policies exist to prevent student overspend and enforce
IT governance — they cannot be overridden by a student user regardless of subscription role.

Every `terraform apply` in the York tenant returned `RequestDisallowedByPolicy` — the infrastructure
could not be built at all. The only path forward was a personal account with full subscription owner
permissions.

Azure for Students provides $100 credit with no credit card required. At YorkPulse's expected
Azure spend (~$10-20/month for Container Apps + ACR + Key Vault), this covers ~5-10 months.
The account is not tied to York University, so it persists after graduation — university accounts
are revoked when enrollment ends.

**Tradeoffs**:
Personal account has no IT support. If the subscription has an issue, resolution goes through
Microsoft support, not a university IT helpdesk. This is acceptable — the subscription is simple
(one resource group, standard services) and Microsoft's personal account support is adequate.

**Interview Talking Point**:
"University cloud tenants enforce IT governance policies that prevent creating the resource types
a real infrastructure project needs. I identified this constraint early, pivoted to a personal
Azure for Students account, and documented the issue. The lesson is to verify cloud permissions
before writing any infrastructure code — a 5-minute 'az group create' test would have caught
this before I wrote any Terraform."

---

## Decision 018 — Service Principal Auth (ARM_* env vars) Over Azure CLI Auth for Terraform

**Decision**: Authenticate the azurerm Terraform provider using a Service Principal with four
ARM_* environment variables instead of using Azure CLI (`az login`) credentials.

**Options Considered**:
1. Azure CLI auth — `az login`, provider reads token from CLI credential cache automatically
2. Service Principal with ARM_* env vars — explicit credentials, CI/CD compatible
3. Managed Identity — automatic auth when running in Azure, not applicable for local dev

**Why This Choice**:
Azure CLI auth has a Mac-specific bug: `az storage` commands and `az group` commands use
different internal authentication paths. After switching tenants via `az account set`, storage
commands silently used a cached token from the old tenant — causing `SubscriptionNotFound`
errors even when `az account show` showed the correct subscription. This bug wasted significant
debugging time during Phase 3 setup.

Service Principal auth with ARM_* env vars:
- Uses explicit credentials that are immune to CLI token cache state
- Works identically on macOS, Linux, and GitHub Actions runners
- Is the standard pattern for CI/CD pipelines (Phase 4 will use the same SP)
- Makes auth explicit and auditable — no hidden CLI session state

The Service Principal (`yorkpulse-terraform`, App ID: `20518fc6-13ab-4f55-87b2-7a6abea05738`)
has Contributor role on the subscription — enough to create/delete resources, but not manage
RBAC assignments (principle of least privilege).

**Required env vars (set in every shell session before running Terraform)**:
```bash
...
**Tradeoffs**:
Service Principal credentials must be kept secure and rotated periodically. The Client Secret
is stored in a password manager — never in code or .env files. GitHub Actions (Phase 4) will
store these as repository secrets and inject them into the pipeline environment.

**Interview Talking Point**:
"I discovered a macOS Azure CLI bug where `az storage` and `az group` use different token caches.
Rather than debugging CLI quirks, I switched to Service Principal auth — which is the correct
pattern for CI/CD anyway. The credentials are explicit, the auth path is the same on every
machine and in every pipeline, and there's no hidden session state to worry about."

---

## Decision 019 — Canada Central Region for All Azure Resources

**Decision**: Deploy all Azure Phase 3 resources in the `canadacentral` Azure region.

**Options Considered**:
1. Canada Central (Toronto) — closest Azure region to York University, lowest latency for users
2. East US — cheaper for some services, closest major US region
3. East US 2 — second cheapest, commonly recommended for North American projects
4. Canada East (Quebec City) — also Canadian but ~500km further from Toronto

**Why This Choice**:
YorkPulse's users are York University students — physically located in Toronto, Ontario.
Canada Central maps to Microsoft's Toronto data centre, which is the closest Azure region
to the university campus. Lower network latency translates directly to faster frontend SSR
response times, which users perceive as page load speed.

Data residency is also relevant: Canadian data stays in Canada. If YorkPulse ever handles
data covered by PIPEDA (Canada's federal privacy law) or Ontario's PHIPA, having data
reside in Canada simplifies compliance — data does not need to cross a US border.

Canada Central supports all services used in Phase 3:
- Container Apps (Microsoft.App) — supported
- Azure Container Registry — supported
- Azure Key Vault — supported
- Azure Log Analytics — supported

**Tradeoffs**:
Canada Central is marginally more expensive than East US for some resource types (ACR Basic:
identical pricing). The latency and data residency benefits outweigh any minor cost difference.

**Interview Talking Point**:
"I chose Canada Central because the users are in Toronto and latency matters for SSR.
A Next.js server-side render that runs in Toronto vs Virginia adds ~35ms of round-trip
network overhead — that's the difference between a fast feel and a sluggish one at
YorkPulse's scale. Data residency is a secondary benefit: Canadian data stays in Canada,
which simplifies future PIPEDA compliance."

---

## Phase 3 — What You Learned (Terraform Azure Infrastructure)

**Always test cloud permissions before writing infrastructure code.**
The York University Azure tenant blocked all resource creation via IT policy. A 30-second
`az group create --name test --location canadacentral` test would have caught this before
writing a single line of Terraform. Test permissions on the target subscription before investing
time in infrastructure code.

**Service Principal auth is superior to CLI auth for infrastructure work.**
The Azure CLI has a Mac-specific bug where `az storage` and `az group` use different token
caches. Service Principal auth (ARM_* env vars) bypasses this entirely, works identically
on every machine, and is the correct pattern for CI/CD pipelines. Start with SP auth — don't
discover the CLI quirks the hard way.

**New Azure subscriptions need provider namespaces registered before first use.**
Microsoft.App (Container Apps) and Microsoft.OperationalInsights (Log Analytics) are not
registered by default. A `terraform apply` on a fresh subscription will fail with a 409
MissingSubscriptionRegistration error until you register these namespaces via `az provider register`.
This one-time setup step is not documented in the azurerm Terraform provider README.

**Container registry + container runtime = two-step deploy.**
ACR must exist before you can push an image. The Container App needs an image at creation time.
Solve with a placeholder image default in variables.tf — first apply bootstraps the registry,
docker push fills it, second apply switches to the real image. This pattern applies to any
cloud: ECR + ECS, ACR + Container Apps, GAR + Cloud Run.

**`data "azurerm_client_config" "current"` resolves the object_id automatically.**
Key Vault access policies require `object_id` (the Service Principal's object in Azure AD),
not `client_id` (the application registration ID). These are different values. The data source
resolves object_id from whoever is currently authenticated via the ARM_* env vars — no
hardcoding, no separate variable, no az CLI lookup needed.
