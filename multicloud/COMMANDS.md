...
## Terraform — Phase 3: Azure Infrastructure (infra/azure/)

> Run these commands from inside the multicloud/infra/azure/ directory.
> Required env vars must be set in your shell before running any Azure Terraform command.
> The Azure Blob Storage backend (yorkpulsetfstate) must already exist.

---

**Command**: Set required Azure environment variables before any Terraform command
```bash
...
export ARM_CLIENT_SECRET="<paste from password manager — never commit this>"
```
**What it does**: Injects Azure credentials into the shell so the azurerm Terraform provider
  and the backend (Azure Blob Storage) can authenticate without any hardcoded values in code.
  These four env vars are the Azure equivalent of AWS_ACCESS_KEY_ID + AWS_SECRET_ACCESS_KEY.
**When you use it**: In every new terminal session before running terraform init/plan/apply.
**Expected output**: No output — env vars are set silently.

---

**Command**: `cd multicloud/infra/azure && terraform init`
**What it does**: Downloads the azurerm provider plugin (~3.0) and connects to the Azure Blob
  Storage remote backend (yorkpulsetfstate container / azure/terraform.tfstate key).
  No Azure resources are created. State is stored in Azure, not locally.
**When you use it**: Once when first entering the infra/azure/ directory.
  Also run after adding a new provider or changing provider versions.
**Expected output**: "Terraform has been successfully initialized!" + backend config confirmation.

---

**Command**: `terraform plan`
**What it does**: Previews all Azure resources that will be created:
  - azurerm_resource_group (yorkpulse-prod, canadacentral)
  - azurerm_container_registry (yorkpulseacr, Basic SKU, admin enabled)
  - azurerm_log_analytics_workspace (30-day retention, PerGB2018)
  - azurerm_container_app_environment (linked to Log Analytics)
  - azurerm_container_app (frontend, 0.5 cpu / 1.0Gi, 1-3 replicas, port 3000)
  - azurerm_key_vault (yorkpulse-prod-kv, standard SKU, full access for SP)
  Nothing is created. Read every resource carefully.
**When you use it**: Always before terraform apply. Never skip.
**Expected output**: "Plan: 6 to add, 0 to change, 0 to destroy."

---

**Command**: `terraform apply`
**What it does**: Creates all Azure infrastructure. Prompts for "yes" confirmation.
  After apply: ACR exists but is empty — push the frontend image before updating var.frontend_image.
  The Container App starts with the placeholder image (Azure sample app) until you push yours.
**When you use it**: After reviewing plan output and confirming all resources look correct.
**Expected output**: "Apply complete! Resources: 6 added, 0 changed, 0 destroyed."
  Followed by outputs: acr_login_server, container_app_url, resource_group_name, key_vault_uri.

---

**Command**: `terraform output`
**What it does**: Prints all outputs from outputs.tf: ACR login server, Container App URL,
  resource group name, Key Vault URI, Log Analytics workspace ID.
  Copy the ACR login server for the docker push commands below.
**When you use it**: After terraform apply completes, or any time you need to recall output values.
**Expected output**: Five key-value pairs with Azure resource identifiers.

---

## Docker — Phase 3 Azure: Build and Push Frontend to ACR

> Run these commands from inside the frontend/ directory.
> ACR must already exist (terraform apply must have run first).
> Authenticate with ACR before every push — ACR tokens expire after 3 hours.

---

**Command**: `az acr login --name yorkpulseacr`
**What it does**: Authenticates Docker with the Azure Container Registry using your az CLI session.
  Equivalent to `aws ecr get-login-password | docker login ...` for AWS ECR.
  Tokens expire after 3 hours — re-run this if docker push returns "unauthorized".
**When you use it**: Before any docker push to ACR. Requires az login to be active first.
**Expected output**: "Login Succeeded"

---

**Command**: `docker buildx build --platform linux/amd64 -t yorkpulseacr.azurecr.io/yorkpulse-frontend:latest --push .`
**What it does**: Builds the Next.js frontend image for linux/amd64 (required — Apple Silicon
  builds ARM64 by default) and pushes it directly to ACR.
  --platform linux/amd64: ensures the image runs on Azure's x86_64 compute nodes.
  --push: skips a separate docker push — builds and pushes in one command.
  Run from the frontend/ directory where the Dockerfile lives.
**When you use it**: After terraform apply (ACR exists) and az acr login (authenticated).
  Run this every time frontend code changes and you want to deploy a new version.
**Expected output**: Build stages completing, then "pushed" with the ACR image digest.

---

**Command**: Update frontend_image variable and re-apply
```bash
terraform apply -var='frontend_image=yorkpulseacr.azurecr.io/yorkpulse-frontend:latest'
```
**What it does**: Updates the Container App to pull the real frontend image instead of the
  Azure placeholder. Triggers a new Container App revision — the old revision drains and
  the new one takes 100% of traffic (Single revision mode).
**When you use it**: After the first successful docker push to ACR.
**Expected output**: "Plan: 0 to add, 1 to change, 0 to destroy." then "Apply complete!"

---

## Azure CLI — Phase 3 Operations

---

**Command**: `az containerapp show --name yorkpulse-frontend --resource-group yorkpulse-prod --query 'properties.latestRevisionFqdn'`
**What it does**: Returns the FQDN (fully qualified domain name) of the latest active revision
  of the frontend Container App. This is the URL you can visit to validate the deployment
  before cutting over DNS.
**When you use it**: After terraform apply or after updating the container image — to get
  the live URL without running terraform output.
**Expected output**: A hostname ending in .azurecontainerapps.io

---

**Command**: `az containerapp update --name yorkpulse-frontend --resource-group yorkpulse-prod --image yorkpulseacr.azurecr.io/yorkpulse-frontend:latest`
**What it does**: Tells the Container App to create a new revision using the latest image tag.
  Equivalent to `aws ecs update-service --force-new-deployment` on the AWS side.
  Useful for pulling a new :latest push without running terraform apply.
  Azure creates a new revision, waits for it to start healthy, then routes 100% traffic to it.
**When you use it**: After pushing a new image to ACR and wanting the Container App to pick it up.
**Expected output**: JSON of the updated Container App configuration showing the new image.

---

**Command**: `az containerapp logs show --name yorkpulse-frontend --resource-group yorkpulse-prod --follow`
**What it does**: Streams live stdout/stderr logs from the running Container App to your terminal.
  Equivalent to `aws logs tail /ecs/yorkpulse-backend --follow` on the AWS side.
  Shows Next.js server startup logs, request logs, and any runtime errors.
**When you use it**: After a deploy to confirm the frontend started cleanly, or when debugging errors.
**Expected output**: Timestamped log lines from the Next.js standalone server.

---

**Command**: `az keyvault secret set --vault-name yorkpulse-prod-kv --name NEXT-PUBLIC-API-URL --value "https://api.yorkpulse.com"`
**What it does**: Stores a secret in Key Vault named NEXT-PUBLIC-API-URL with the ECS backend URL.
  Key Vault secret names cannot contain underscores — use hyphens instead of underscores.
  Stored secrets can later be injected into Container App env vars as Key Vault references (Phase 5).
**When you use it**: After terraform apply creates the vault — populate all frontend secrets here.
**Expected output**: JSON confirming the secret was created with its vault URI and version ID.

---

**Command**: Register Azure provider namespaces (one-time setup for new subscriptions)
```bash
az provider register --namespace Microsoft.App
az provider register --namespace Microsoft.OperationalInsights
# Wait ~1-2 minutes, then verify both show "Registered":
...
```
**What it does**: Enables the Container Apps (Microsoft.App) and Log Analytics (Microsoft.OperationalInsights)
  namespaces on a new Azure subscription. New subscriptions do not auto-register these providers.
  Without this step, `terraform apply` fails with 409 MissingSubscriptionRegistration on both the
  Container App Environment and the Log Analytics workspace.
**When you use it**: Once, on any new Azure subscription before running `terraform apply`.
  Already done on subscription fb7e60ee — do not need to repeat unless you create a new sub.
**Expected output**: `az provider show` returns "Registered" for both namespaces.

---

**Command**: `az keyvault secret list --vault-name yorkpulse-prod-kv --query '[].name' --output table`
**What it does**: Lists all secret names stored in the Key Vault (names only, not values).
  Use this to verify all expected secrets are present after populating the vault.
**When you use it**: After populating secrets to confirm they all exist.
**Expected output**: A table of secret names.
...
**What it does**: Pushes commits to the main branch, which triggers the full deploy pipeline
  in GitHub Actions. The pipeline runs three jobs in sequence:
  1. security-scan — Bandit + Safety + Trivy + Checkov (non-blocking)
  2. deploy-backend — Build + push to ECR → ECS redeploy → smoke test
  3. deploy-frontend — Build + push to ACR → Azure Container App update → smoke test
**When you use it**: Every time you want to deploy a new version to production
**Expected output**: Green checkmarks in the GitHub Actions tab. Backend smoke test returns 200.
  Frontend smoke test returns 200.

---

**Command**: `git push origin main` (with no backend/frontend/multicloud changes)
**What it does**: Push is ignored by the pipeline — the `paths` filter in deploy.yml means the
  pipeline only triggers when backend/, frontend/, or multicloud/ files change.
  Pushes that only touch README.md or docs/ do NOT trigger a deploy.
**When you use it**: Docs-only changes — safe to push without triggering a redeploy
**Expected output**: No Actions workflow run appears for that commit

---

**View pipeline**: github.com/blacks1k-sc/YorkPulse/actions
**What it does**: Shows all workflow runs — current, queued, and historical. Click any run to
  see per-job logs, timing, and scan findings from Bandit/Trivy/Checkov.
**When you use it**: After every push to monitor deploy progress or investigate failures

---

**Command**: Rollback backend to a previous image
```bash
# List recent ECR image tags to find the SHA you want to roll back to
aws ecr list-images \
...
# Force ECS to use the previous :latest (re-push the old SHA tag as :latest)
docker pull 062677866920.dkr.ecr.us-east-1.amazonaws.com/yorkpulse-backend:<old-sha>
docker tag  062677866920.dkr.ecr.us-east-1.amazonaws.com/yorkpulse-backend:<old-sha> \
            062677866920.dkr.ecr.us-east-1.amazonaws.com/yorkpulse-backend:latest
docker push 062677866920.dkr.ecr.us-east-1.amazonaws.com/yorkpulse-backend:latest
aws ecs update-service \
  ...
  --region us-east-1
```
**What it does**: Re-tags a previous immutable image as :latest and forces ECS to redeploy it.
  Every deploy tags images with both :latest AND :<github-sha>, so any commit is rollback-able.
**When you use it**: When a bad deploy reaches production and you need to revert immediately

---

**Command**: Rollback frontend to a previous image
```bash
az containerapp update \
  --name yorkpulse-frontend \
  --resource-group yorkpulse-prod \
  --image yorkpulseacr.azurecr.io/yorkpulse-frontend:<old-sha>
```
**What it does**: Points the Azure Container App at a specific immutable image by SHA tag.
**When you use it**: When the frontend deploy introduces a regression

---

**Command**: Check GitHub Actions secrets are configured
**What it does**: Verify these 5 secrets exist in the repo before the first pipeline run:
  - AWS_ROLE_ARN (the GitHub Actions IAM role ARN from AWS)
  - AZURE_CLIENT_ID (Service Principal App ID)
  - AZURE_CLIENT_SECRET (Service Principal password)
  - AZURE_TENANT_ID (Azure AD tenant ID)
  - AZURE_SUBSCRIPTION_ID (Azure subscription ID)
**Where to set them**: github.com/blacks1k-sc/YorkPulse → Settings → Secrets and variables → Actions
