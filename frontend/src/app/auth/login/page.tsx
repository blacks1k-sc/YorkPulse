...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!email.endsWith("@yorku.ca") && !email.endsWith("@my.yorku.ca")) {
      toast({
        title: "Invalid email",
        description: "Please use your @yorku.ca or @my.yorku.ca email",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      const response = await loginMutation.mutateAsync(email);
      ...
      router.push(`/auth/check-email?email=${encodeURIComponent(email)}${tokenParam}`);
    } catch (error) {
      toast({
        title: "Error",
